...
#Calc_NLCD function does all landscape metric calculations for all future folders
...
#Calc_NLCD function does all landscape metric calculations for all future folders
calc_NLCD_historical<-function(name, year){
  ...
  #NLCD (selected by year and scenario combination)
  nlcd<-raster(paste0("Data/Forecast_NLCD/CONUS_Landcover_Historical/", name, ".tif"))
  ...
  result<-foreach(c=1:length(counties), .combine=rbind)%dopar%{ #For each county
    ...
    finaldf<-cbind(allmat,natural_crops_mat,forest_crop_mat,shrubgrass_crop_mat, countydf) %>% #bind all calculations
      dplyr::mutate(Year=year, #add year column,
                    ...
                    forest_crops_ed=tel_forestcrop_fc/CountyArea_ha,#add meters shared edge per hectare county area for forest_crops
                    shrubgrass_crops_ed=tel_shrubgrasscrop_sgc/CountyArea_ha) %>% #add meters shared edge per hectare county area for shrub_grass
      dplyr::select(FIPS, Year, everything()) #FIPS, Year, and Scenario columns first
    finaldf[is.na(finaldf)] <- 0 #Add replace NA with zero (for calculations)
...
#Current
  #2001 (Need to use historical function)-Will be identical so will just copy them
    #B1
    B1_2001<-calc_NLCD_historical(name="CONUS_Historical_y2001",year=2001) %>% 
      dplyr::mutate(Scenario="B1") %>% #Add scenario column
      dplyr::select(FIPS, Year, Scenario, everything())#arrange so same as other years
    write_csv(B1_2001, "Data/nlcd_forecast_current/B1_2001.csv")
    #B2
    B2_2001<-B1_2001 %>% 
      dplyr::mutate(Scenario="B2") %>% #Add scenario column
      dplyr::select(FIPS, Year, Scenario, everything())#arrange so same as other years
    write_csv(B2_2001, "Data/nlcd_forecast_current/B2_2001.csv")
    #A1B
    A1B_2001<-B1_2001 %>% 
      dplyr::mutate(Scenario="A1B") %>% #Add scenario column
      dplyr::select(FIPS, Year, Scenario, everything())#arrange so same as other years
    write_csv(A1B_2001, "Data/nlcd_forecast_current/A1B_2001.csv")
    #A2
    A2_2001<-B1_2001 %>% 
      dplyr::mutate(Scenario="A2") %>% #Add scenario column
      dplyr::select(FIPS, Year, Scenario, everything())#arrange so same as other years
    write_csv(A2_2001, "Data/nlcd_forecast_current/A2_2001.csv")
  #2006 
    #B1
    B1_2006<-calc_NLCD(scenario="B1",year=2006) 
    write_csv(B1_2006, "Data/nlcd_forecast_current/B1_2006.csv")
    #B2
    B2_2006<-calc_NLCD(scenario="B2",year=2006)
    write_csv(B2_2006, "Data/nlcd_forecast_current/B2_2006.csv")
    #A1B
    A1B_2006<-calc_NLCD(scenario="A1B",year=2006)
    write_csv(A1B_2006, "Data/nlcd_forecast_current/A1B_2006.csv")
    #A2
    A2_2006<-calc_NLCD(scenario="A2",year=2006)
    write_csv(A2_2006, "Data/nlcd_forecast_current/A2_2006.csv")
  #2011
    #B1
    B1_2011<-calc_NLCD(scenario="B1",year=2011) 
    write_csv(B1_2011, "Data/nlcd_forecast_current/B1_2011.csv")
    #B2
    B2_2011<-calc_NLCD(scenario="B2",year=2011)
    write_csv(B2_2011, "Data/nlcd_forecast_current/B2_2011.csv")
    #A1B
    A1B_2011<-calc_NLCD(scenario="A1B",year=2011)
    write_csv(A1B_2011, "Data/nlcd_forecast_current/A1B_2011.csv")
    #A2
    A2_2011<-calc_NLCD(scenario="A2",year=2011)
    write_csv(A2_2011, "Data/nlcd_forecast_current/A2_2011.csv")
  #2016
    #B1
    B1_2016<-calc_NLCD(scenario="B1",year=2016) 
    write_csv(B1_2016, "Data/nlcd_forecast_current/B1_2016.csv")
    #B2
    B2_2016<-calc_NLCD(scenario="B2",year=2016)
    write_csv(B2_2016, "Data/nlcd_forecast_current/B2_2016.csv")
    #A1B
    A1B_2016<-calc_NLCD(scenario="A1B",year=2016)
    write_csv(A1B_2016, "Data/nlcd_forecast_current/A1B_2016.csv")
    #A2
    A2_2016<-calc_NLCD(scenario="A2",year=2016)
    write_csv(A2_2016, "Data/nlcd_forecast_current/A2_2016.csv")
...
nlcd_current<-rbind(B1_2001, B2_2001, A1B_2001, A2_2001,
                    B1_2006, B2_2006, A1B_2006, A2_2006,
                    B1_2011, B2_2011, A1B_2011, A2_2011,
                    B1_2016, B2_2016, A1B_2016, A2_2016)%>%
    dplyr::select(FIPS, Year, Scenario, everything()) %>% 
    arrange(FIPS)
  
write_csv(nlcd_current, "Data/nlcd_forecast_current/nlcd_forecast_current.csv")
