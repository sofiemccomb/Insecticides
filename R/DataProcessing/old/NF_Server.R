...
states<-sort(states)
  ...
  for (c in counties){
    ...
    if(length(unique(rc_deciforest_LID))>0){
      #Fragstat class and landscape metrics
      tec<-as.data.frame(lsm_c_te(rc_deciforest_LID))
      tel<-as.data.frame(lsm_l_te(rc_deciforest_LID))
    ...
    if(length(unique(rc_mixforest_LID))>0){
      #Fragstat class and landscape metrics
      tec<-as.data.frame(lsm_c_te(rc_mixforest_LID))
      tel<-as.data.frame(lsm_l_te(rc_mixforest_LID))
