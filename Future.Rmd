...
#Wrangle current timeseries data for model input
  data_model_na<-fulldata %>% 
    dplyr::select(FIPS, Year, ASDCode, ERSCode,
           ...
           annual_GDD, tmin_avg_JanFebMar, tmax_avg_JunJulAug) %>% #Select variables
    na.omit() #Get rid of rows with NAS
  dat_fips<- data_model_na%>% #Get the FIPS for a balanced panel (all 4 yrs) for current data
      group_by(FIPS) %>%
      summarise(count=n()) %>% 
...
#Sum of differences
  ...
  for(i in scenarios){
    ...
    colnames(df_diff_planted)<-c("PercentCrop_diff", "LargeCropPatches_diff", "SoyGrains_diff", "Corn_diff",
                                 ...
                                 "WinterTminavg_diff", "SummerTmaxavg_diff") #Rename columns
...
B1 is the global environmental suistabintility with more environmental focus globally;
A2 is the regionally oriented economic developement with regionslism that is more economic focus;


A2 is the divided world while B1 is an integrated ecologically friendly world

Sohl 2014 paper
A2-ag land will doulbe in area in the US by 2100; so perhaps less insecticide decrease because much greater ag land increase
"Quantity of ag land in A1B and A2 scenarios would far surpass any historical extent of ag land use"-
nearly 3 million km2 of new cropland between 2000 adn 2100 in the A2 scenario-marginal lands
What IMAGE demanded vs what they actually implemented
