...
data_model_1_na<-fulldata %>% 
  ...
  filter(!insect_planted>1&!soysmallgrain_planted>1&!corn_planted>1&!fruitveg_planted>1) %>% 
  na.omit() %>% #11362 observations %>% 
    select(FIPS, Year, ASDCode, ERSCode,
         insect_planted, planted_county, largefarm_planted, 
...
data_model_1<-left_join(dat1_fips, data_model_1_na)%>%#Balanced the panel, 10500 observations
...
dat2_fips<- data_model_2_na%>%
  ...
  filter(count==4) %>% 
  select(FIPS) #Get FIPS FOR BALANCED PANEL
data_model_2<-left_join(dat2_fips, data_model_2_na)%>%#Balanced the panel, 10500 observations
  mutate_at(scale, .vars=vars(-FIPS, -Year, -ERSCode, -ASDCode, -insect_planted)) %>%  #Scale by subtracting column mean and dividing by column SD
    rename(Insecticide=insect_planted,
         PlantedCrop=planted_county,
         LargeFarms=largefarm_planted,
...
dat3_fips<- data_model_3_na%>%
  ...
  filter(count==3) %>% 
  select(FIPS) #Get FIPS FOR BALANCED PANEL
data_model_3<-left_join(dat3_fips, data_model_3_na)%>%#Balanced the panel, 8097 observations
  mutate_at(scale, .vars=vars(-FIPS, -Year, -ERSCode, -ASDCode, -insect_planted)) %>%  #Scale by subtracting column mean and dividing by column SD
    rename(Insecticide=insect_planted,
         PlantedCrop=planted_county,
         LargeFarms=largefarm_planted,
...
dat4_fips<- data_model_4_na%>%
  ...
  filter(count==4) %>% 
  select(FIPS) #Get FIPS FOR BALANCED PANEL
data_model_4<-left_join(dat4_fips, data_model_4_na)%>%#Balanced the panel, 10500 observations
  mutate_at(scale, .vars=vars(-FIPS, -Year, -ERSCode, -ASDCode, -insect_planted)) %>%  #Scale by subtracting column mean and dividing by column SD
    rename(Insecticide=insect_planted,
...
dat5_fips<- data_model_5_na%>%
  ...
  filter(count==4) %>% 
  select(FIPS) #Get FIPS FOR BALANCED PANEL
data_model_5<-left_join(dat5_fips, data_model_5_na)%>%#Balanced the panel, 10500 observations
  mutate_at(scale, .vars=vars(-FIPS, -Year, -ERSCode, -ASDCode, -insect_planted)) %>%  #Scale by subtracting column mean and dividing by column SD
    rename(Insecticide=insect_planted,
         PercentCrop=pland_crops,
         LargeCropPatches=Prop_LCP,
...
dat6_fips<- data_model_6_na%>%
  ...
  filter(count==4) %>% 
  select(FIPS) #Get FIPS FOR BALANCED PANEL
data_model_6<-left_join(dat6_fips, data_model_6_na)%>%#Balanced the panel, 10552 observations
  mutate_at(scale, .vars=vars(-FIPS, -Year, -ERSCode, -ASDCode, -insect_planted)) %>%  #Scale by subtracting column mean and dividing by column SD
    rename(Insecticide=insect_planted,
         PercentCrop=pland_crops,
         LargeCropPatches=Prop_LCP,
...
dat7_fips<- data_model_7_na%>%
  ...
  filter(count==4) %>% 
  select(FIPS) #Get FIPS FOR BALANCED PANEL
data_model_7<-left_join(dat7_fips, data_model_7_na)%>%#Balanced the panel, 10552 observations
  mutate_at(scale, .vars=vars(-FIPS, -Year, -ERSCode, -ASDCode, -insect_planted)) %>%  #Scale by subtracting column mean and dividing by column SD
    rename(Insecticide=insect_planted,
         PercentCrop=pland_crops,
         LargeCropPatches=Prop_LCP,
...
stargazer::stargazer(lm_model_1, lm_model_2, lm_model_3, lm_model_4,lm_model_5,lm_model_6,lm_model_7,
                     ...
                     out="Data/Present/stargazer/7Models_stargazer_planted_balanced_robustness_noinsectcropover1.txt",
