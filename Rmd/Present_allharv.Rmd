...
data_model_1<-fulldata %>% 
  select(FIPS, Year, ASDCode, ERSCode,
         ...
         soysmallgrain_harv, corn_harv, fruitveg_harv,msidi,
  ...
  na.omit() %>% #11405 observations %>% 
    select(FIPS, Year, ASDCode, ERSCode,
         insect_harv, harv_county, largefarm_harv, 
         soysmallgrain_harv, corn_harv, fruitveg_harv) %>% #remove msidi so scaled correctly
...
data_model_4<-fulldata %>% 
  select(FIPS, Year, ASDCode, ERSCode,
         insect_harv, pland_crops, Prop_LCP, 
         soysmallgrain_harv, corn_harv, fruitveg_harv,
         natural_crops_ed, msidi, harv_county, largefarm_harv) %>% #Include harv_county and largefarm so exact same rows
  na.omit() %>% #11816 observations
   select(FIPS, Year, ASDCode, ERSCode,
         insect_harv, pland_crops, Prop_LCP, 
         soysmallgrain_harv, corn_harv, fruitveg_harv,
         natural_crops_ed, msidi) %>% #Remove harv_county and largefarm_harv so scaled correctly
  mutate_at(scale, .vars=vars(-FIPS, -Year, -ERSCode, -ASDCode, -insect_harv)) %>%  #Scale by subtracting column mean and dividing by column SD
    rename(Insecticide=insect_harv,
...
data_model_5<-fulldata %>% 
  select(FIPS, Year, ASDCode, ERSCode,
         insect_harv, pland_crops, Prop_LCP, 
         soysmallgrain_harv, corn_harv, fruitveg_harv,
         natural_crops_ed, msidi, harv_county, largefarm_harv) %>% #Include harv_county and largefarm and crops so exact same rows
  na.omit() %>% #11816 observations
   select(FIPS, Year, ASDCode, ERSCode,
         insect_harv, pland_crops, Prop_LCP,
         natural_crops_ed, msidi) %>% #Remove harv_county and largefarm_harv and crops so scaled correctly
  mutate_at(scale, .vars=vars(-FIPS, -Year, -ERSCode, -ASDCode, -insect_harv)) %>%  #Scale by subtracting column mean and dividing by column SD
    rename(Insecticide=insect_harv,
         PercentCrop=pland_crops,
         LargeCropPatches=Prop_LCP,
...
stargazer::stargazer(lm_model_1, lm_model_2, lm_model_3, lm_model_4,lm_model_5,lm_model_6,lm_model_7,
                     ...
                     out="Data/Present/stargazer/7Models_stargazer_allharv.txt",
