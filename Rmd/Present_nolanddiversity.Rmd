...
###Model 2
Census of Ag (Harv crop, large farms, crop covariates) with naturalcropedge for 4 years
...
data_model_2<-fulldata %>% 
  ...
  na.omit() %>% #11816 observations
    select(FIPS, Year, ASDCode, ERSCode,
         insect_planted, harv_county, largefarm_planted, 
         soysmallgrain_planted, corn_planted, fruitveg_planted,
         natural_crops_ed) %>% #Need to scale for only the variables selecting
  mutate_at(scale, .vars=vars(-FIPS, -Year, -ERSCode, -ASDCode, -insect_planted)) %>%  #Scale by subtracting column mean and dividing by column SD
    rename(Insecticide=insect_planted,
         HarvestedCrop=harv_county,
         LargeFarms=largefarm_planted,
...
###Model 3
Census of Ag (Harv crop, large farms, crop covariates) with natural cropedge and crop diversity for only 3 years (no 2002)-only one with crop diversity
...
###Model 4
NLCD equivalents (Percent cropland, LCP patch metric) with naturalcropedg and crop covariates for 4 years (no large farms)
...
data_model_4<-fulldata %>% 
  ...
  na.omit() %>% #11816 observations
   select(FIPS, Year, ASDCode, ERSCode,
         insect_planted, pland_crops, Prop_LCP, 
         soysmallgrain_planted, corn_planted, fruitveg_planted,
         natural_crops_ed) %>% #Remove harv_county and largefarm_planted so scaled correctly
  mutate_at(scale, .vars=vars(-FIPS, -Year, -ERSCode, -ASDCode, -insect_planted)) %>%  #Scale by subtracting column mean and dividing by column SD
    rename(Insecticide=insect_planted,
...
data_model_5<-fulldata %>% 
  ...
  na.omit() %>% #11816 observations
   select(FIPS, Year, ASDCode, ERSCode,
         insect_planted, pland_crops, Prop_LCP,
         natural_crops_ed) %>% #Remove harv_county and largefarm_planted and crops so scaled correctly
  mutate_at(scale, .vars=vars(-FIPS, -Year, -ERSCode, -ASDCode, -insect_planted)) %>%  #Scale by subtracting column mean and dividing by column SD
    rename(Insecticide=insect_planted,
         PercentCrop=pland_crops,
         LargeCropPatches=Prop_LCP,
...
###Model 6
Modeled NLCD (forecast for 4 years for pcrop, patch metric, naturalcropedge) with crop covariates
...
stargazer::stargazer(lm_model_1, lm_model_2, lm_model_3, lm_model_4,lm_model_5,lm_model_6,lm_model_7,
                     ...
                     out="Data/Present/stargazer/7Models_stargazer_nolanddiversity.txt",
