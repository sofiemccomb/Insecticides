...
load("Data/Future/DataProcessing/df/fulldata_forecast.rda")
...
###Model 1 
Census of Ag (Harv crop, large farms, crop covariates) for 4 years, all observations possible (no <1%)-all observations applies to all models
...
###Model 2
Census of Ag (Harv crop, large farms, crop covariates) with cropedge and land diversity for 4 years
...
###Model 3
Census of Ag (Harv crop, large farms, crop covariates) with cropedge and land diversity and crop diversity for only 3 years (no 2002)-only one with crop diversity
...
data_model_3<-fulldata %>% 
  select(FIPS, Year, ASDCode, ERSCode,
         insect_planted, harv_county, largefarm_planted, 
         soysmallgrain_planted, corn_planted, fruitveg_planted,
         ed_crops, msidi, SDI,pland_crops, Prop_LCP) %>% 
...
lm_model_3<-lm(Insecticide~HarvestedCrop+LargeFarms+
                 SoyGrains+Corn+FruitVeg+
                 CropEdge+LandDiversity+ CropDiversity+
...
###Model 4
NLCD equivalents (Percent cropland, LCP patch metric) with cropedge and land diversity and crop covariates for 4 years (no large farms)
...
data_model_4<-fulldata %>% 
  select(FIPS, Year, ASDCode, ERSCode,
         ...
         soysmallgrain_planted, corn_planted, fruitveg_planted,
         ed_crops, msidi, harv_county, largefarm_planted) %>% #Include harv_county and largefarm so exact same rows
  na.omit() %>% #11924 observations
   select(FIPS, Year, ASDCode, ERSCode,
         insect_planted, pland_crops, Prop_LCP, 
         soysmallgrain_planted, corn_planted, fruitveg_planted,
         ed_crops, msidi) %>% #Remove harv_county and largefarm_planted so scaled correctly
  mutate_at(scale, .vars=vars(-FIPS, -Year, -ERSCode, -ASDCode, -insect_planted)) %>%  #Scale by subtracting column mean and dividing by column SD
    rename(Insecticide=insect_planted,
...
data_model_5<-fulldata %>% 
  select(FIPS, Year, ASDCode, ERSCode,
         insect_planted, pland_crops, Prop_LCP, 
         soysmallgrain_planted, corn_planted, fruitveg_planted,
         ed_crops, msidi, harv_county, largefarm_planted) %>% #Include harv_county and largefarm and crops so exact same rows
  na.omit() %>% #11924 observations
   select(FIPS, Year, ASDCode, ERSCode,
         insect_planted, pland_crops, Prop_LCP,
         ed_crops, msidi) %>% #Remove harv_county and largefarm_planted and crops so scaled correctly
  mutate_at(scale, .vars=vars(-FIPS, -Year, -ERSCode, -ASDCode, -insect_planted)) %>%  #Scale by subtracting column mean and dividing by column SD
    rename(Insecticide=insect_planted,
         PercentCrop=pland_crops,
         LargeCropPatches=Prop_LCP,
...
stargazer::stargazer(lm_model_1, lm_model_2, lm_model_3, lm_model_4,lm_model_5,lm_model_6,lm_model_7,
                     ...
                     out="Data/Present/stargazer/7Models_stargazer_cropedges.txt",
