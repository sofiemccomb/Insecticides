...
stargazer(lm_nocrops_4yrs)
#Create sample stargazer tables, by running desired lm outputs from Analysis (rerun Analysis, do not want to save since takes a lot of storage)
#Will need to change out SE and p-values for those from CRSE results
#dont' need F statistic-just observations and R2



stargazer(lm_nocrops_4yrs,lm_nocrops_3yrs, lm_crops_4yrs, lm_crops_3yrs, omit=c("ASDCode","Year"), 
          type = 'text', out = 'Data/Visualize/stargazer/samplestargazer.txt',
...
se = robust[,2], p = robust[,4], p.auto = F
df_coef_nocrops_4yrs
se<-c(list(HAC1), list(HAC2),...)



cov1        <- vcovHC(lm_nocrops_4yrs, type = "HC0")
robust1     <- sqrt(diag(cov1))
head(robust1)

list(robust1, robust1)
list(df_coef_nocrops_3yrs[,3])
```
